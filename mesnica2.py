...
with st.sidebar:
    ...
    st.markdown("<h2 style='text-align: center; font-size: 20px;'>KOJUNDŽIĆ MESNICA</h2>", unsafe_allow_index=True)
    st.write("---")
    izbor = st.radio("NAVIKACIJA", ["🛍️ TRGOVINA", "🛒 KOŠARICA", "ℹ️ O NAMA & KONTAKT"])
    st.write("---")
    st.caption("Sisak, Hrvatska | v2.1")

# --- STRANICA: TRGOVINA ---
if izbor == "🛍️ TRGOVINA":
    st.title("Premium Ponuda")
    st.info("ℹ️ **Napomena o vaganju:** Svi proizvodi prodaju se na kilograme. Točan iznos narudžbe znat će se nakon vaganja.")
    ...
    for i, p in enumerate(proizvodi):
        with cols[i % 2]:
            ...
            qty = st.number_input(f"Tražena količina (kg)", min_value=0.0, step=0.1, key=f"q_{p['id']}", format="%.1f")
...
# --- STRANICA: KOŠARICA ---
    ...
    if not st.session_state.cart:
        st.info("Košarica je prazna. Vratite se u trgovinu i odaberite domaće meso.")
    else:
        ukupno = 0
        detalji_mail = ""
        ...
        st.markdown("---")
        st.markdown(f"<h2 style='text-align: right;'>Ukupno: cca {ukupno:.2f} €</h2>", unsafe_allow_index=True)
        st.caption("Plaćanje nakon vaganja (pouzećem ili uplatom). Dostava unutar EU.")

        with st.form("forma_narudzbe"):
            st.markdown("### 🚚 Podaci za dostavu")
            ime = st.text_input("Ime i Prezime*")
            adr = st.text_input("Ulica, poštanski broj i Grad*")
            ...
            if st.form_submit_button("PRIPREMI NARUDŽBU"):
                if ime and adr and mob:
                    ...
                    st.error("Ispunite obavezna polja označena s *.")

# --- STRANICA: O NAMA ---
elif izbor == "ℹ️ O NAMA & KONTAKT":
    st.title("Tradicija Kojundžić")
    st.image("images.unsplash.com", use_container_width=True)
    st.write("""
    **Mesnica i prerada mesa Kojundžić** obiteljski je obrt u kojem se meso obrađuje po starim recepturama sisačke Posavine.
    Naš fokus je na kvaliteti, domaćem uzgoju i tradicionalnom dimljenju.
    """)
    st.divider()
    st.subheader("📍 Gdje nas pronaći?")
